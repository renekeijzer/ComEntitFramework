...
namespace std {
	template <>
	struct hash<cef::entity::Entity> {
		std::size_t operator()(const cef::entity::Entity& k) const { return k.id().id(); }
	};
}
...
struct Body : cef::entity::Component<Body> {
...
struct Renderable : cef::entity::Component<Renderable> {
...
struct Fadeable : cef::entity::Component<Fadeable> {
...
struct Collideable : cef::entity::Component<Collideable> {
...
struct CollisionEvent : public cef::event::Event<CollisionEvent> {
	CollisionEvent(cef::entity::Entity left, cef::entity::Entity right) : left(left), right(right) {}

	cef::entity::Entity left, right;
};


// Updates a body's position and rotation.
struct BodySystem : public cef::systems::System<BodySystem> {
	void update(cef::entity::EntityManager &es, cef::event::EventManager &events, cef::TimeDelta dt) override {
		Body::Handle body;
		for (cef::entity::Entity entity : es.entities_with_components(body)) {
...
struct FadeOutSystem : public cef::systems::System<FadeOutSystem> {
	void update(cef::entity::EntityManager &es, cef::event::EventManager &events, cef::TimeDelta dt) override {
		Fadeable::Handle fade;
		Renderable::Handle renderable;
		for (cef::entity::Entity entity : es.entities_with_components(fade, renderable)) {
...
class BounceSystem : public cef::systems::System<BounceSystem> {
public:
...
class CollisionSystem : public cef::systems::System<CollisionSystem> {
...
private:
	...
	void collect(cef::entity::EntityManager &entities) {
		Body::Handle body;
		Collideable::Handle collideable;
		for (cef::entity::Entity entity : entities.entities_with_components(body, collideable)) {
			unsigned int
	...
	void collide(cef::event::EventManager &events) {
...
class ExplosionSystem : public cef::systems::System<ExplosionSystem>, public cef::event::Receiver<ExplosionSystem> {
public:
	void configure(cef::event::EventManager &events) override {
		events.subscribe<CollisionEvent>(*this);
	}

	void update(cef::entity::EntityManager &es, cef::event::EventManager &events, cef::TimeDelta dt) override {
		for (cef::entity::Entity entity : collided) {
	...
	void emit_particles(cef::entity::EntityManager &es, cef::entity::Entity entity) {
		...
		for (int i = 0; i < area; i++) {
			cef::entity::Entity particle = es.create();
...
private:
	std::unordered_set<cef::entity::Entity> collided;
};


// Render all Renderable entities and draw some informational text.
class RenderSystem :public cef::systems::System<RenderSystem> {
public:
	...
	void update(cef::entity::EntityManager &es, cef::event::EventManager &events, cef::TimeDelta dt) override {
		Body::Handle body;
		Renderable::Handle renderable;
		for (cef::entity::Entity entity : es.entities_with_components(body, renderable)) {
...
class Application : public cef::Bootstrap {
public:
	explicit Application(sf::RenderTarget &target, sf::Font &font) {
		...
		for (int i = 0; i < 500; i++) {
			cef::entity::Entity entity = entities.create();
